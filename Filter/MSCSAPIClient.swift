...
class MSCSAPIClient {
    ...
    static let endpointBaseURL = "https://yishengli.cognitiveservices.azure.com/vision/v3.0/"
    ...
    func analyzeImage(_ image: UIImage, detectFeatures featureTypes: [FeatureTypes], completion:@escaping (AnyObject?, Error?) -> Void) {
        ...
        URLSession.shared.dataTask(with: request, completionHandler: { (data, response, error) in
            ...
            guard let jsonObject = ((try? JSONSerialization.jsonObject(with: responseData, options: []) as? [String: AnyObject]) as [String : AnyObject]??) else {
                let error = NSError(domain: "MSCSAPIClient", code: MSCSAPIClientError.jsonFormatError.rawValue, userInfo: [NSLocalizedDescriptionKey:"Could not decode JSON data from response."])
                DispatchQueue.main.async(execute: {
                    completion(nil, error)
                })
                return
